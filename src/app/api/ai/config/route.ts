...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select(
        'provider, model, system_prompt, is_active, auto_reply_enabled, auto_reply_max_per_conversation, suggestions_enabled, handoff_agent_id, api_key, embeddings_api_key, base_url',
      )
...
export async function POST(request: Request) {
  try {
    ...
    const suggestionsEnabled = body.suggestions_enabled === true
    ...
    if (credentialsChanged) {
      try {
        await validateAiCredentials({
          ...
          // Irrelevant to a credentials check — this literal only
          // exists to satisfy the AiConfig shape for the ping.
